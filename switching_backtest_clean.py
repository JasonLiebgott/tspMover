...
# Requirements: pandas, numpy, yfinance, pandas_datareader, matplotlib, scipy
...
REBALANCE_FREQ = 'ME' # monthly rebalance (end of month)
...
# Strategy thresholds (as discussed) - CAPE functionality removed
THRESHOLDS = {
...
METRIC_WEIGHTS = {
    'sahm_rule': 0.20,        # Highest weight - best predictor
    'yield_curve': 0.15,      # Strong recession signal
    'jobless_claims': 0.15,   # Real-time data
    'lei_index': 0.12,        # Comprehensive leading indicator
    'ism_pmi': 0.10,          # Business sentiment
    'gdp_growth': 0.10,       # Economic reality
    'sp500_ma200': 0.08,      # Market technical
    'vix_level': 0.05,        # Market stress
    'credit_spreads': 0.03,   # Credit conditions
    'core_pce': 0.02          # Policy implications
}
...
cpi_core_m = cpi_core.resample('ME').last().ffill()
ten_year_m = ten_year.resample('ME').last().ffill()
try:
    t10_breakeven_m = t10_breakeven.resample('ME').last().ffill()
except Exception:
    t10_breakeven_m = None

unemp_m = unemp.resample('ME').last().ffill()
if ism is not None:
    ism_m = ism.resample('ME').last().ffill()
else:
    ism_m = None

# Compose signal table (CAPE-related signals removed)
...
try:
    dgs3 = fetch_fred_series('DGS3MO', START, END).resample('ME').last().ffill()
...
for idx, row in signal_df.iterrows():
    ...
    # Valuation layer - CAPE functionality removed, only basic checks remain
    # For now, valuation defaults to NEUTRAL since CAPE is removed
    val_flags['valuation_flag'] = 'NEUTRAL'
...
# -----------------------
def layer_color(row):
    # Valuation: since CAPE is removed, valuation defaults to NEUTRAL
    val = row['valuation_flag']
...
bh_val = bh_val.reindex(portfolio_history.index).ffill().bfill()
...
protection_share = alloc_sub.apply(lambda x: x['protection'])
...
plt.title('Switching Framework Backtest (CAPE Functionality Removed)')
plt.legend()
plt.xlabel('Date')
